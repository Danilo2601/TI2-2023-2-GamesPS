...
public class JogoService {
	...
	public void makeForm(int tipo, Jogo jogo,int orderby) {
	    ...
	    Path filePath = resourcePath.resolve("inserir.html"); 
		...
		for (Jogo p : jogos) {
			...
			list += "\n<tr bgcolor=\"" + bgcolor + "\">\n" +
					...
					"\t<td align=\"center\" valign=\"middle\"><a href=\"javascript:confirmarDeleteJogo('" + p.getId() + "', '" + p.getNome() + "', '" + p.getEmpresa() + "');\"><img src=\"/image/delete.png\" width=\"20\" height=\"20\"/></a></td>\n" +
					"</tr>\n";
		}
	...
	public Object insert(Request request, Response response) {
    	...
    	ArrayList<Integer> categers = new ArrayList<Integer>();
    	
    	for(int i = 0; i < categ.length; i++) {
    		categers.add(Integer.parseInt(categ[i]));
    	}
    	
		
		String resp = "";
		
		Jogo jogo = new Jogo(-1, nome,categers,empresa,descricao,avaliacao,links,especificacoes,imagens);
	...
	public Object update(Request request, Response response) {
        ...
        if (jogo != null) {
        	...
        	for(int i = 0; i < conv.length; i++) {
        		categers.add(Integer.parseInt(conv[i]));
        	}
        	
        	jogo.setCategorias(categers);
